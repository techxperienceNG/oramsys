...
const Paginate = ({ postsPerPage, totalPosts, getCountries, paginate, prevPagefunc, currentPage, nextPagefunc }) => {
  const pageNumbers = [];
  const ellipsis = '...';
  ...
  return (
    <div className='d-flex align-items-center justify-content-between' aria-label="...">
      <div className='pe-4'>
        <span className="text-muted fs-6 fw-light">Showing {getCountries?.length} out of {totalPosts}</span>
      </div>
